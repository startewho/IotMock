...
- **发送 / 接收消息日志**：主界面底部「消息日志」面板，可**折叠 / 展开 / 清空**。实时记录收到的请求与发送的响应，区分 **接收 / 发送**、显示 **功能码 / 字节数（寄存器个数）/ 完整消息 16 进制**，每条可**一键复制完整消息**。
- **Modbus 请求 / 消息生成弹框**：主界面「Modbus 发送」按钮打开生成器，选择**功能码（读写）/ 数据类型 / 字节序**，输入事务 ID、单元 ID、起始地址、数量及（写入时的）数值 / 字符串 / 位串，生成**请求 PDU** 与**完整发送帧（MBAP + PDU）**，均带**复制到剪贴板**按钮。
