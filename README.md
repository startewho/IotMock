...
- **实时修改数据（Bit 位编辑）**：双击可写数据区的任意行，弹出 **Bit 位编辑弹窗**：
  - **16 位**（单寄存器）：用 16 个 CheckBox 勾选对应位；
  - **32 位**（双寄存器，占 2 个连续地址）：用 32 个 CheckBox 编辑；
  - 实时十六进制 / 十进制预览，确认后立即写入共享存储并刷新表格。
